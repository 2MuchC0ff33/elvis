...
This runbook documents the operational configuration and quick reference for
running the Elvis scraper.
...
1. Set a runtime override locally (temporary):
   ...
   export FETCH_TIMEOUT=10 ./bin/elvis-run

2. Permanent operational default (project-level): edit `project.conf`:

   FETCH_TIMEOUT=15

3. Secrets (API keys, notification credentials): place only in `.env` or use a
...
Use a small parser that ignores comments and blank lines, then exports KEY=VALUE
pairs. Example (POSIX sh compatible):
...
- Avoid simply `source`-ing files that may contain unexpected code; prefer the
  safe parse above.
- After loading, scripts may apply per-seed overrides by checking for
  `OVERRIDE_${seed_id}_KEY` environment variables or parsing
  `seek-pagination.ini`.
